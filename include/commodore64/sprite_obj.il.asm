...
//   - Grouped, non-destructive read-modify-write on VIC-II global registers
...
//   $92/$93  sobj_zp_an    anim-table pointer (scratch inside sobj_tick only)
//
//   All three ranges are from the "safe" zones documented in Constants.asm.
//   This library does NOT touch zp_tmp ($4e/$4f), zp_str2 ($02/$03),
//   zp_ptr_2 ($64/$65), TEMP_7 ($04), or TEMP_8 ($05).
//
// ── Usage example ─────────────────────────────────────────────────────────────
//
...
// Apply object to VIC-II and reset state.  Call once to initialise.
...
// Does NOT write to any VIC-II register — no sprite glitches at any raster line.
...
// Write animated sprite pointers to VIC-II.
...
// color: VIC-II color value (e.g. WHITE, RED, GREEN …)
...
// Call from the main loop after jsr wait_vbl (writes VIC-II X/Y registers).
...
//--------------------------------------------------------------------
// sobj_apply
// Write all object settings to the VIC-II; reset the state block.
// In:  sobj_zp_obj ($5c/$5d) → sprite object data
//      sobj_zp_st  ($5e/$5f) → state block (9 bytes)
// Clobbers: A, X, Y
// Does NOT touch: zp_tmp, zp_str2, zp_ptr_2, TEMP_7, TEMP_8
//--------------------------------------------------------------------
sobj_apply:
...
//--------------------------------------------------------------------
// sobj_tick
// Advance animation frame counters ONLY.  No VIC-II register writes.
...
// Does NOT touch: sobj_zp_obj/st/an, zp_tmp, zp_str2, TEMP_7, TEMP_8
//--------------------------------------------------------------------
sobj_tick:
...
//--------------------------------------------------------------------
// sobj_flush
// Write current animation frame pointers to VIC-II sprite registers.
// Call from the MAIN LOOP after jsr wait_vbl so writes always land
// during VBlank, away from the VIC-II sprite DMA window.
// In:  sobj_zp_obj ($5c/$5d) → sprite object data
